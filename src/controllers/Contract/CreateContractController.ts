...
class CreateContractController {
    async handle(req: Request, res: Response) {
        ...
        const order = await createContractService.execute({
            userId, services, risk, lifes, name, services_gestao, company, contact, consultant, phone_number, banking, discount, service_name, service_value, service_description, life_value, initial_value
        })
